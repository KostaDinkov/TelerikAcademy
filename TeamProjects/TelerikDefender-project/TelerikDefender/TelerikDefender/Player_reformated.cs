//Note: this is a part of a teamwork project for C# part 2 course at Telerik Academy written back in 2013
//This one was written with no OOP course taken, so do not judge harshly :)
...
namespace TelerikDefender
{
    ...
    {
        private string[] graphic2 = System.IO.File.ReadAllLines(Environment.CurrentDirectory +
            "\\" + "Assets" +
            "\\" + "Graphics" +
            "\\" + "ninjaSwipe.txt");
        ...
        private void SwipeWithSword()
        {
            Sounds.PlaySwordSwipe();
            this.Clear();
            this.Draw(graphic2);
            this.isSwiping = true;
            ...
            if (this.flyingShurs.Count > 0)
            {
                foreach (var shuriken in this.flyingShurs)
                {
                    ...
                    if (shuriken.col + shuriken.objectWidth / 2 == Game.Enemy.col &&
                        (shuriken.row + 3 >= Game.Enemy.row && 
                        shuriken.row + 3 <= Game.Enemy.row + Game.Enemy.objectHight))
                    {
